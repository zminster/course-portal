...
connection.query("SELECT user_meta.first_name, user_meta.last_name, assignment.name, grades.asgn_id, assignment_meta.date_due, assignment.url, user_meta.email FROM grades INNER JOIN membership ON grades.uid = membership.uid LEFT JOIN assignment ON grades.asgn_id = assignment.asgn_id INNER JOIN assignment_meta ON grades.asgn_id = assignment_meta.asgn_id AND membership.class_pd = assignment_meta.class_pd LEFT JOIN user ON grades.uid = user.uid LEFT JOIN user_role ON user.role = user_role.rid LEFT JOIN user_meta ON user.uid = user_meta.uid WHERE handed_in=0 AND chomped=0 AND displayed=1 AND can_handin=1 AND user_role.handin_enabled = 1 AND user_role.reporting_enabled = 1 AND grades.nreq = 0 AND date_due < NOW() AND trimester = (SELECT value_int FROM system_settings WHERE name = 'current_trimester');", async(err, gradeRows) => {
