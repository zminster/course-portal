<?php
	...
	function get_all_userinfo($conn) {
		$res = $conn->query("SELECT user.uid, role, class_pd, username, first_name, last_name, year, email, advisor_email FROM user LEFT JOIN membership ON user.uid = membership.uid INNER JOIN user_meta ON user.uid = user_meta.uid ORDER BY role, class_pd, username");
		$users = array();
